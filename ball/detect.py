...
def infer_ball_on_video(
    ...
    cap.release()
    dt = time.time() - t0
    print(f"Done. Video: {video_path} | frames: {total_frames} | sampled: {sampled_count} | stride: {stride} | time: {dt:.1f}s")
...
def _rebuild_from_cache(cache_dir, model_id, confidence, vid_fps, width, height) -> List[Dict]:
    """Rebuilds combined jsonl from existing per-frame json cache."""
...
def _run_inference_loop(cap, stride, model_id, confidence, max_frames, cache_dir, save_frame_json, vid_fps, width, height, client, total_frames) -> Tuple[List[Dict], int]:
    """Iterates through video frames, performs inference, and collects results."""
    records = []
    frame_idx = 0
    sampled_count = 0
...
def main():
    ...
    parser.add_argument("--model-id", default=os.getenv("ROBOFLOW_MODEL_ID", "volleyball_v2/3"))
    parser.add_argument("--confidence", type=float, default=settings.OVERLAY_MIN_CONF)
    parser.add_argument("--infer-fps", type=float, default=settings.INFER_FPS)
    ...
    infer_ball_on_video(
        ...
        combined_jsonl=settings.BALL_DETECTIONS_JSONL,
        save_jpegs=settings.BALL_SAVE_JPEGS,
        save_frame_json=settings.BALL_SAVE_FRAME_JSON,
