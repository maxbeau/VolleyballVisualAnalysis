...
def soft_weight_aspect_ratio(
    best: Dict[int, Dict[str, Any]], min_ar: float, max_ar: float, alpha: float
    ...
    for k, p in best.items():
        ...
        try:
            ...
            if ar <= 0.0:
                ...
                t = (min_ar - ar) / max(min_ar, 1e-6)
                weight = float(np.exp(-alpha * t))
            elif ar > max_ar:
                t = (max_ar - ar) / max(max_ar, 1e-6)
                weight = float(np.exp(-alpha * t))
            if weight < 1.0:
...
def run_trajectory_analysis(
    *,
    ...
    ensure_dir(os.path.dirname(output_path_img) or ".")
    ...
    if not os.path.exists(homography_npy) or not os.path.exists(homography_meta_json):
        raise FileNotFoundError(f"Court homography not found. Searched for: {homography_npy}")
    H = np.load(homography_npy)
    with open(homography_meta_json, "r", encoding="utf-8") as f:
        H_meta = json.load(f)
    ...
    best = soft_weight_aspect_ratio(best, ar_filter_min, ar_filter_max, ar_filter_alpha)
    ...
    # Image-space smoothing (with gating/gravity/hold)
    ...
    for i in range(total_frames):
        ...
        if p is None or float(p.get("confidence", 0.0)) < min_confidence:
            continue
    ...
    bg = None
    if os.path.exists(birdseye_jpg):
        bg = cv2.imread(birdseye_jpg)
    if bg is None:
    ...
    print(f"Trajectory JSONL: {output_jsonl}")
    print(f"Trajectory CSV:   {output_csv}")
    print(f"Bird's-eye path:  {output_path_img}")
