...
def infer_actions_on_video(
    ...
    """Run Roboflow volleyball-actions model over the video with sampling and cache outputs.
    ...
    dt = time.time() - t0
    print(
        f"Actions detection done. Video: {video_path} | frames: {total_frames} | sampled: {sampled_count} | stride: {stride} | time: {dt:.1f}s"
    )
...
def _rebuild_from_cache(cache_dir, model_id, confidence, vid_fps, width, height) -> List[Dict]:
    ...
    for idx, jp in cached_files:
        with open(jp, "r", encoding="utf-8") as jf:
            result = json.load(jf)
        preds = _extract_predictions_from_result(result)
        records.append({
            "frame": idx,
...
def _extract_predictions_from_result(result: Dict) -> List[Dict]:
    """Try to normalize Roboflow result to a list of prediction dicts.

    - If detection-style (has 'predictions' with boxes), return as-is.
    - If classification-style, attempt to convert to a list of {'class','confidence'}.
    """
...
def main():
    parser = argparse.ArgumentParser(description="Detect volleyball actions using Roboflow with per-frame cache and JSONL output")
